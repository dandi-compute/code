...
To compare capsules by what they hold rather than where they sit, `identical` fingerprints each one from the content addressed blob every asset points at, so no download is needed: `python migrate_job_capsule_names.py identical` groups capsules holding the same outputs, and `--compare code` or `--compare all` widens the scope. No scope compares a capsule whole, since its `dataset_description.json` carries its own job ID and its submission markers carry the time they were submitted, which would make every capsule unique; both are always left out. Identical contents mean the same work was done twice rather than that either capsule is wrong, so the phase only reports.
