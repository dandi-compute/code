...
If the archive ends up holding the same migrated capsule in two Dandisets, `duplicates` finds them: `python migrate_job_capsule_names.py duplicates` reports each one with what either side holds (asset count, whether it has outputs or logs), and `--remove-from <dandiset id>` deletes them from that side, on the archive and in the local clone. A capsule belongs to one Dandiset — the job capsules one while live, the failed runs archive once archived — but which side is the leftover is not something the paths can say, so nothing is deleted until you name it. Only migrated capsules are compared; a legacy path in both is what `clean` handles.
